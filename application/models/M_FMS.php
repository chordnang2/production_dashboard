...
class M_FMS extends CI_Model
{
    public function get($unit, $date)
    {
        ...
        if (empty($model['detail_cycle'])) {
            return array("unit" => $unit, "date" => $date, "message" => "Unit tidak beroperasi (Tidak ada data)");
        } else {
            $flag_loading = 0;
            foreach ($model['detail_cycle'] as $key => $value) {
            ...
            if ($flag_loading == 0) {
                return array("unit" => $unit, "date" => $date, "message" => "Unit tidak melakukan loading (Kemungkinan Breakdown)");
            } elseif ($flag_loading == 1) {
                return array("unit" => $unit, "date" => $date, "message" => "Unit hanya melakukan 1x loading (Seharusnya minimal 2x)");
            }
        }
    ...
    public function insertCycleWarning($data)
    {
        if ($data) {
            $this->db->trans_start();

            foreach ($data as $key => $value) {
                $delete_sql = "DELETE FROM msg_cycle_warning WHERE unit LIKE '" . $value['unit'] . "' AND date LIKE '" . $value['date'] . "'";
                $this->db->query($delete_sql);
                $sql = "INSERT INTO msg_cycle_warning VALUES ('" . $value['unit'] . "', '" . $value['date'] . "', '" . $value['message'] . "')";
                $this->db->query($sql);
            }
            $this->db->trans_complete();
        }
    ...
    function addTime($time1, $time2)
    {
        ...
        foreach ($times as $time) {
            ...
            if (strpos($hour, "days") !== false) {
                $temp_explode = explode(' ', $hour);
                $days = $temp_explode[0] * 24;
                $hour = $temp_explode[2] + $days;
            }
            $seconds += $hour * 3600;
            $seconds += $minute * 60;
            $seconds += $second;
    ...
    function get_cycle_summary($tanggal)
    {

        $query = $this->db->query("SELECT *,DATE_FORMAT(loading_in, '%H:%i:%s') as load_awal,DATE_FORMAT(loading_out, '%H:%i:%s') as load_akhir FROM `cycle_summary` WHERE  date='$tanggal' AND total_milage>=130");
    ...
    function get_cycle_transaksi($idsatu, $iddua, $unit)
    {


        $query = $this->db->query("SELECT * FROM `tbl_fms_geofence` JOIN tbl_lokasi ON tbl_fms_geofence.geofence=tbl_lokasi.id_lokasi  WHERE unit like '$unit' AND id BETWEEN '$idsatu' AND '$iddua'");
        return $query->result_array();
    }
    function get_bug_cycle($tanggal)
    {


        $query = $this->db->query("SELECT * FROM `msg_cycle_warning` WHERE  date like '$tanggal'");
        return $query->result_array();
    }

    function summary_fms($bulan)
    {
        $query = $this->db->query("SELECT
    AVG(TIME_TO_SEC(total_duration)) as totdur_11,
      AVG(TIME_TO_SEC(total_durasiparkir)) as totparkir_11,
      AVG(TIME_TO_SEC(total_loading)) as totload_11,
      AVG(TIME_TO_SEC(total_tipping)) as tottip_11,
      AVG(TIME_TO_SEC(total_travel)) as tottrav_11,
      AVG(TIME_TO_SEC(total_parkingbay)) as totparb_11,
      AVG(TIME_TO_SEC(total_pitstop)) as totppit_11,
      AVG(TIME_TO_SEC(total_refuelling)) as totref_11,
      AVG(TIME_TO_SEC(total_weighbridge)) as totwb_11,
      AVG(TIME_TO_SEC(total_workshop)) as totwork_11,
      DATE_FORMAT(date, '%d') as day_11
  FROM
      cycle_summary
  WHERE
      DATE_FORMAT(date, '%Y-%m') = '$bulan' AND total_milage>=130
  group by
      day_11
  order by
      day_11 ASC");
    ...
    public function wb_hanson($data = null, $limit = null, $not = null, $order = null)
    {
        ...
        if (!empty($data)) {
            ...
            $sql = "SELECT * FROM cycle_summary WHERE 1=1 $condition $order $limit";
    ...
    function max_timein_geofence()
    {
        $query = $this->db->query("SELECT max(DATE_FORMAT(time_in,'%Y-%m-%d'))- INTERVAL 1 DAY as kemaren FROM `tbl_fms_geofence` WHERE time_in !='-----'");
        return $query->result_array();
    }
}
